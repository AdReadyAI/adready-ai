...
Deno.test("parseScoreEngineRequest accepts valid metric_results", () => {
  ...
  if (parsed.ok) {
    ...
    assertEquals(parsed.metric_results[0].confidence, "high");
  }
});

Deno.test("parseScoreEngineRequest accepts audience_channel_fit", () => {
  const parsed = parseScoreEngineRequest({
    metric_results: [
      {
...
Deno.test("parseScoreEngineRequest rejects removed v0.2 metric_ids", () => {
  const parsed = parseScoreEngineRequest({
    metric_results: [
