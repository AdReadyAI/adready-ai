...
import { parseLLMJson, TriageResponseSchema } from "../../../functions/claims-agent/tools/llm-response.ts";

const goodArray = JSON.stringify([
  { claim_id: "c1", is_verifiable_claim: true, category: "factual_claim", reasoning: "x" },
]);
...
Deno.test("parseLLMJson: throws a clear error on non-JSON prose", () => {
  assert.throws(
    () => parseLLMJson("Sure, here is the analysis: claim-1 looks fine.", TriageResponseSchema, "test"),
...
Deno.test("parseLLMJson: throws on valid JSON with the wrong top-level shape", () => {
  assert.throws(
    () => parseLLMJson(JSON.stringify({ not: "an array" }), TriageResponseSchema, "test"),
...
Deno.test("parseLLMJson: throws on an invalid enum value", () => {
  const bad = JSON.stringify([
    { claim_id: "c1", is_verifiable_claim: true, category: "not_a_real_category", reasoning: "x" },
  ]);
  assert.throws(
