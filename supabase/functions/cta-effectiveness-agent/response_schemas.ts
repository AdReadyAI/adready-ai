/**
 * response_schemas.ts — Zod schemas + parsing for the CTA agent's two LLM replies.
 *
 ...
 * detected_ctas[] as proof of absence. Call 2 returns the shared evaluation
 * envelope. A reply that does not match parses to null (via `safeParseJson`)
 * and the agent degrades to cannot_assess.
 *
...
// primitive), and each carries the numeric start_ms/end_ms of the segment it was
// found in, so the positional checks stay deterministic arithmetic over those
// timestamps.
...
// Aliases: the CTA agent's Call 2 uses the shared evaluation envelope.
