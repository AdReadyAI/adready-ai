/**
 * context.ts — DB-backed AgentContext loader boundary for the Product
...
export function loadAgentContext(_requestId: string): Promise<AgentContext> {
  // TODO(db): load product frames, logo frames, visual frames, transcript, OCR,
  // product context, and video metadata from Supabase by request_id, then
  // validate with AgentContextSchema before returning.
  throw new Response(
    JSON.stringify({
      ok: false,
      error: {
