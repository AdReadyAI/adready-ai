-- Complete evaluation inside Supabase once every canonical evaluator metric
-- exists for an Ad Creative. Railway remains responsible only for Media
-- Processing and never receives scorecard projection work.
alter table public.requests
  add column evaluation_completion_status text,
  add column evaluation_completion_attempts integer not null default 0,
  add column evaluation_completion_started_at timestamptz,
  add column evaluation_completion_completed_at timestamptz,
  add column evaluation_completion_last_error text,
...
declare
  ...
  trigger_secret text;
begin
  -- Unrelated evaluator output does not affect the canonical Score Engine
  -- input and therefore must not schedule completion.
  ...
  -- Serialize completion scheduling per Ad Creative. When one evaluator
  -- replaces multiple metrics in a transaction, its first row marks the
  -- request pending and subsequent row triggers become no-ops.
  ...
  exception when others then
    -- Scheduling failure must not roll back evaluator output. Persist the
    -- failure so an operator can correct configuration and safely retry.
    update public.requests
