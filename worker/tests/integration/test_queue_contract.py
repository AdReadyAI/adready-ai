...
@pytest.mark.integration
def test_atomic_metric_completion_enqueues_one_score_job() -> None:
    """Only the tenth scoring-relevant parent metric completes the score job."""
    database_url = os.environ.get(
        "TEST_DATABASE_URL",
        "postgresql://postgres:postgres@127.0.0.1:54322/postgres",
    )
    ...
    def score_job_count(cursor) -> int:
        """Count queued score jobs for only this disposable Review Request."""
        cursor.execute(
            """
            SELECT count(*)
            FROM pgmq.q_jobs
            WHERE message->>'job_type' = 'score'
        ...
        with connection.cursor() as cursor:
            try:
                cursor.execute(
                    ...
                    """,
                    (user_id, f"score-trigger-{user_id}@example.invalid"),
                )
                cursor.execute(
                    """
                    INSERT INTO requests (request_id, batch_id, user_id)
                    VALUES (%s, %s, %s);
                    """,
                    (request_id, batch_id, user_id),
                )

                # Each evaluator result is independently persisted. The first
                # nine must leave the score queue untouched.
                for agent, metric_id in atomic_metrics[:9]:
                    cursor.execute(
                        """
                        INSERT INTO agent_results (
                          request_id, agent, metric_id, metric_name,
                          result, severity
                        ) VALUES (%s, %s, %s, %s, 'true', 'none');
                        """,
                        (request_id, agent, metric_id, metric_id),
                    )
                assert score_job_count(cursor) == 0

                agent, metric_id = atomic_metrics[9]
                cursor.execute(
                    """
                    INSERT INTO agent_results (
                      request_id, agent, metric_id, metric_name,
                ...
                assert score_job_count(cursor) == 1

                # Evidence and sub-checks enrich issues but do not change the
                # canonical scoring inputs, so neither may enqueue again.
                cursor.execute(
                    """
                    INSERT INTO agent_result_evidence (
                      request_id, agent, metric_id, evidence_order,
                      evidence_type, evidence_text
                    ) VALUES (%s, %s, %s, 0, 'text', 'Evidence');
                    """,
                    (request_id, agent, metric_id),
                )
                cursor.execute(
                    """
                    INSERT INTO agent_result_sub_checks (
                      request_id, agent, metric_id, check_id, name,
                      result, severity
                    ) VALUES (%s, %s, %s, 'check', 'Check', 'true', 'none');
                    """,
                    (request_id, agent, metric_id),
                )
                assert score_job_count(cursor) == 1
