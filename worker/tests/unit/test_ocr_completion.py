...
def test_completion_run_scopes_text_segment_provenance(tmp_path):
    """Durable Text Segment identities belong to their owning OCR Run."""
    candidate_path = tmp_path / "representative.jpg"
    cv2.imwrite(
        str(candidate_path),
        np.zeros((20, 40, 3), dtype=np.uint8),
    ...
    analysis = FixedRateOcrAnalysis(
        segments=(
            OcrSegment(
                ...
                source_text_segment_ids=("text_segment_0001",),
            ),
        ),
        representative_candidates=(representative,),
        text_segments=(
    ...
    assert completion.text_segments[0].identifier == (
        "ocr-run-123-text-segment-0001"
    )
    assert completion.ocr_segments[0].source_text_segment_ids == (
        "ocr-run-123-text-segment-0001",
    )
