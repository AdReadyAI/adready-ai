...
*   `detected_claims[]`: Primitive list of claims from Media Processing.
...
2.  **Stage 2: Claim Triage (Claude Haiku)**: Classify claims and filter out puffery.
3.  **Stage 3: Claim Substantiation (Claude Sonnet)**: Audit claims vs the creative brief.
4.  **Stage 4: Compliance Check (Claude Sonnet/Opus)**: Evaluate legal warnings.
...
2.  **Stage 2: Hook Assessment (Claude Sonnet)**: Audit hook impact in opening scenes.
...
3.  **Stage 3: Visibility Check (Claude Sonnet)**: Audit sizing and contrast.
...
2.  **Stage 2: Vision Analysis (Claude Sonnet)**: Audit packaging visual fidelity against references.
...
*   `keyframes[]`: Keyframes for visual audit.
*   `ocr_segments[]`: OCR rendering confidence scores.
*   `scene_segments[]`: Boundary cuts.
...
3.  **Stage 3: Vision Checks (Claude Sonnet)**: Audit frames for visual quality, artifacts, and lighting.
...
2.  **Stage 2: Placement & Palette Checks (Claude Sonnet)**: Audit typography, layout, and palette matching.
...
2.  **Stage 2: Message Adherence Check (Claude Sonnet)**: Audit text against creative brief messages.
3.  **Stage 3: Demographic Fit Check (Claude Sonnet)**: Audit style elements against audience habits.
