...
* *Visual freeze-frames extracted from scene boundaries or intervals. Each keyframe contains:*
* **`frame_id`**: String — Unique keyframe identifier (e.g., `"frame_001"`).
* **`timestamp_ms`**: Integer — Exact timestamp where the frame was captured.
* **`scene_id`**: String — ID of the scene this keyframe belongs to.
* **`image_url`**: String — Fully accessible CDN or Supabase Storage URL (no raw base64 arrays allowed).
...
* **`frame_ids`**: Array of Strings — List of associated keyframe IDs showing the product (used by visual agents to verify packaging).
